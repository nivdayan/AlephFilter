...
public class ExperimentsBase {
    ...
    public static long[] generateRandomPayload(int payload_size) {
        Random random = new Random();
        long[] array = new long[(payload_size + Long.SIZE - 1) / Long.SIZE];
        int red_payload_size = payload_size % Long.SIZE;
        long bound;
        for (int i = 0; i < array.length; i++) {
            array[i] = random.nextLong();
            if (red_payload_size > 0 && i == 0) {
                bound = 1L << red_payload_size - 1;
            } else {
                bound = 1L << Long.SIZE - 1;
            }
            array[i] &= bound;
        }
