...
public class QuotientFilter extends Filter implements Cloneable {
    ...
    void set_fingerprint(long index, long fingerprint) {
        filter.setFromTo(get_start_of_slot(index) + 3, (long) get_start_of_slot(index) + 3 + fingerprintLength, fingerprint);
    }

    // sets the payload for a given slot index

    public long get_start_of_fingerprint_mirroring() {
        return get_logical_num_slots_plus_extensions() * bitPerEntry;
    }

    public long get_start_of_payload(long index) {
        if (Constants.getPayloadStrategy() == PayloadStrategy.NEIGHBORING)
            return index * (bitPerEntry + payloadSize) + 3 + fingerprintLength;
        else
            return get_start_of_fingerprint_mirroring() + index * payloadSize;
    }
    ...
    public String get_pretty_str_neighboring(boolean vertical) {
    ...
    public String get_pretty_str_mirroring(boolean vertical) {
        StringBuffer sbr = new StringBuffer();
        String ss = String.format("%-" + fingerprintLength + "s", "FP");
        // More beautiful columnar rep
        sbr.append("\ni\t   \t").append(ss);
        ...
        sbr.append("\n\nidx\tPayloads");
        int counter = 0;
        for (; i < all_slots * payloadSize + get_start_of_fingerprint_mirroring(); i++) {
            //for (long i = 0; i < 100; i++) {
            if ((i - get_start_of_fingerprint_mirroring()) % payloadSize == 0)
                sbr.append('\n').append(Integer.toString(counter++)).append('\t');
            sbr.append(filter.get(i) ? "1" : "0");
        }
        sbr.append('\n');
        return sbr.toString();
    ...
    public String get_pretty_str(boolean vertical) {
        if (Constants.getPayloadStrategy() == PayloadStrategy.NEIGHBORING)
            return get_pretty_str_neighboring(vertical);
        else
            return get_pretty_str_mirroring(vertical);
    }

    // print a nice representation of the filter that can be humanly read.
    public void pretty_print() {
        System.out.println(get_pretty_str(true));
    }

    // return a fingerprint in a given slot index
    long get_fingerprint(long index) {
        return filter.getFromTo(get_start_of_slot(index) + 3, get_start_of_slot(index) + 3 + fingerprintLength);
    }
    ...
    long[] get_slot(long index) {
        long bitsAndFingerprint;
        if (Constants.getPayloadStrategy() == PayloadStrategy.NEIGHBORING)
            bitsAndFingerprint = filter.getFromTo(index * (bitPerEntry + payloadSize), index * (bitPerEntry + payloadSize) + bitPerEntry);
        else
            bitsAndFingerprint = filter.getFromTo(index * (bitPerEntry), index * (bitPerEntry) + bitPerEntry);
    ...
    long get_start_of_slot(long index) {
        if (Constants.getPayloadStrategy() == PayloadStrategy.NEIGHBORING)
            return index * (bitPerEntry + payloadSize);
        else
            return index * bitPerEntry;
    }

    boolean is_occupied(long index) {
        return filter.get(get_start_of_slot(index));
    }

    boolean is_continuation(long index) {
        return filter.get(get_start_of_slot(index) + 1);
    }

    boolean is_shifted(long index) {
        return filter.get(get_start_of_slot(index) + 2);
    }

    void set_occupied(long index, boolean val) {
        filter.set(get_start_of_slot(index), val);
    }

    void set_continuation(long index, boolean val) {
        filter.set(get_start_of_slot(index) + 1, val);
    }

    void set_shifted(long index, boolean val) {
        filter.set(get_start_of_slot(index) + 2, val);
    }
