...
public class QuotientFilter extends Filter implements Cloneable {
    ...
    public String get_pretty_str(boolean vertical) {
        ...
        for (; i < all_slots * payloadSize + get_start_of_fingerprint_mirroring(); i++) {
            //for (long i = 0; i < 100; i++) {
            if ((i - get_start_of_fingerprint_mirroring()) % payloadSize == 0) sbr.append('\n').append(Integer.toString(counter++)).append('\t');
            sbr.append(filter.get(i) ? "1" : "0");
        }
        sbr.append('\n');
        return sbr.toString();
    ...
    long[] get_payload(long index) {
        ...
        long startPosition = get_start_of_payload(index);
        // System.out.println(startPosition + " -- " + bitPerEntry + " " + payloadSize + " " + fingerprintLength + " " + index);
    ...
    long[] get_slot(long index) {
        long bitsAndFingerprint = filter.getFromTo(index * (bitPerEntry), index * (bitPerEntry) + bitPerEntry);
