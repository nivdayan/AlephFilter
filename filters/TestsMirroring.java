...
public class TestsMirroring {
    ...
    static public void test_integ_payload() {
        ...
        qf.insert(fingerprint0, 7, false, new long[]{14});
        qf.insert(fingerprint0, 1, false, new long[]{10});
...
//	// here we test the rejuvenation operation of InfiniFilter
    ...
    static public void test14() {
        ...
        qf.insert(fp1, 1, false, new long[]{1});

        qf.expand();

        qf.insert(fp3, 5, false, new long[]{8});

        qf.insert(fp2, 5, false, new long[]{7});
    ...
    static public void test_insertions_and_deletes(BasicInfiniFilter qf) {
        ...
        for (int i = 0; i < num_entries_to_insert; i++) {
            ...
            if (!added.contains(rand_num)) {
                boolean success = qf.insert(rand_num, false, new long[]{i});
                if (success) {
                    added.add(rand_num);
...
//
    static public void test21(int trials) {
        ...
        for (int i = 0; i < trials; i++) {
            ...
            boolean first_cond = !(qf.insert(input_int, false, new long[]{i}));
            boolean second_cond = qf.search(input_int);
            long[] tmp_arr = qf.delete(input_int);
            boolean third_cond = tmp_arr[tmp_arr.length - 1] == -1;
            boolean last_cond = qf.search(input_int);
            if (first_cond ||
                    !(second_cond) || third_cond ||
                    last_cond) {
            ...
            first_cond = !(qf.insert(input_long, false, new long[]{i}));
            second_cond = qf.search(input_long);
            tmp_arr = qf.delete(input_long);
            third_cond = tmp_arr[tmp_arr.length - 1] == -1;
            last_cond = qf.search(input_long);
            if (first_cond ||
                    !(second_cond) || third_cond || last_cond) {
            ...
            first_cond = !(qf.insert(input_string, false, new long[]{i}));
            second_cond = qf.search(input_string);
            tmp_arr = qf.delete(input_string);
            third_cond = tmp_arr[tmp_arr.length - 1] == -1;
            last_cond = qf.search(input_string);
            if (first_cond ||
                    !(second_cond) || third_cond || last_cond) {
            ...
            rnd.nextBytes(input_bytes);
            first_cond = !(qf.insert(input_bytes, false, new long[]{i}));
            second_cond = qf.search(input_bytes);
            tmp_arr = qf.delete(input_bytes);
            third_cond = tmp_arr[tmp_arr.length - 1] == -1;
            last_cond = qf.search(input_bytes);
            if (first_cond ||
                    !(second_cond) || third_cond || last_cond) {
